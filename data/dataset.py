...
class Dataset(object):
    ...
    def batch_sentences(self, sentences, lang_id):
        ...
        if USE_TAG:
            lengths = torch.LongTensor([len(s) + 4 for s in sentences])
        else:
        ...
        name = self.params.id2lang[lang_id]
        if '1' in name:
            domain_id = 0
        elif '2' in name:
            domain_id = 1
        ...
        if USE_TAG:
            sent[1] = self.params.domain_index[domain_id]
            sent[2] = self.params.task_index[task_id]

        for i, s in enumerate(sentences):
            if USE_TAG: 
                sent[3:lengths[i] - 1, i].copy_(s)
            else:
