...
def set_parameters(params, dico):
    ...
    bos_index = [dico.index(SPECIAL_WORD % (1)) for i in range(params.n_langs)]
    domain_index = [dico.index(SPECIAL_WORD % (i + 2)) for i in range(4)]
    task_index = [dico.index(SPECIAL_WORD % (i + 6)) for i in range(2)]
...
def check_all_data_params(params):
    ...
    params.langs = sorted(params.langs)
    ...
    for (lang1, lang2), (train_path, valid_path, test_path) in params.para_dataset.items():
        ...
        #print(valid_path.replace('XX',lang1))
