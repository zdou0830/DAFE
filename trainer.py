...
class TrainerMT(MultiprocessingEventLoop):
    ...
    def word_shuffle(self, x, l, lang_id):
        ...
        if USE_TAG:
            noise[1] = -1  # do not move start sentence symbol
            noise[2] = -1  # do not move start sentence symbol
    ...
    def word_dropout(self, x, l, lang_id):
        ...
        if USE_TAG:
            keep[1] = 1  # do not drop the start sentence symbol
            keep[2] = 1  # do not drop the start sentence symbol
    ...
    def word_blank(self, x, l, lang_id):
        ...
        if USE_TAG:
            keep[1] = 1  # do not blank the start sentence symbol
            keep[2] = 1  # do not blank the start sentence symbol
    ...
    def drop_tags(self, x, l):
        """
        Randomly blank input words.
        """
        # define words to blank
        lengths = []
        x2 = torch.LongTensor(l.max()-2, l.size(0)).fill_(self.params.pad_index)
        bos_index = self.params.bos_index[0]
        for i in range(l.size(0)):
            words = x[:l[i] - 1, i].tolist()
            # randomly drop words from the input
            new_s = [words[0]] + words[3:]
            new_s.append(self.params.eos_index)
            assert new_s[0] == bos_index and new_s[-1] == self.params.eos_index
            temp_l2 = len(new_s)
            x2[:temp_l2, i].copy_(torch.LongTensor(new_s))
            lengths.append(temp_l2)
        l2 = torch.LongTensor(lengths)
        return x2, l2
    ...
    def enc_dec_step(self, lang1, lang2, lambda_xe, back=False):
        ...
        if USE_TAG:
            sent2, len2 = self.drop_tags(sent2, len2)
