...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.domain_embeds = nn.ModuleList([Embedding(max(3, self.padding_idx+1), embed_dim, self.padding_idx) for _ in range(self.n_langs)])
        self.task_embeds = nn.ModuleList([Embedding(max(3, self.padding_idx+1), embed_dim, self.padding_idx) for _ in range(self.n_langs)])
        ...
        for k in range(args.encoder_layers):
            self.domain_layers.append(Embedding(max(3, self.padding_idx+1), embed_dim, self.padding_idx))
            self.task_layers.append(Embedding(max(3, self.padding_idx+1), embed_dim, self.padding_idx))
    ...
    def forward(self, src_tokens, src_lengths, lang_id, in_domain=False, translation=True):
        assert type(lang_id) is int

        domain_id = 0 if in_domain else 1
        task_id = 0 if translation else 1

        embed_tokens = self.embeddings[lang_id]

        domain_embed_tokens = self.domain_embeds[lang_id]
        domain_tokens = torch.ones_like(src_tokens) * domain_id
        domain_tokens[src_tokens == self.padding_idx] = self.padding_idx

        task_embed_tokens = self.task_embeds[lang_id]
        task_tokens = torch.ones_like(src_tokens) * task_id
        task_tokens[src_tokens == self.padding_idx] = self.padding_idx

        # embed tokens and positions
        domain_x = self.embed_scale * domain_embed_tokens(domain_tokens)
        task_x = self.embed_scale * task_embed_tokens(task_tokens)
        ...
        for i, layer in enumerate(self.layers):
            domain_x = self.embed_scale * self.domain_layers[i](domain_tokens)
            task_x = self.embed_scale * self.task_layers[i](task_tokens)
