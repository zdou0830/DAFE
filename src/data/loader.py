...
def load_para_in_data(params, data):
    """
    Load in-domain parallel data.
    """
    assert len(params.para_in_dataset) > 0

    for (lang1, lang2), paths in params.para_in_dataset.items():
        ...
        assert (lang1, lang2) not in data['para_in']
        data['para_in'][(lang1, lang2)] = {k: v for k, v in datasets}

    logger.info('')

def load_para_out_data(params, data):
    """
    Load out-of-domain parallel data.
    """
    assert len(params.para_out_dataset) > 0

    for (lang1, lang2), paths in params.para_out_dataset.items():
        ...
        assert (lang1, lang2) not in data['para_out']
        data['para_out'][(lang1, lang2)] = {k: v for k, v in datasets}
...
def load_mono_in_data(params, data):
    """
    Load in-domain monolingual data.
    """
    assert not (len(params.mono_in_dataset) == 0) ^ (params.n_mono == 0)
    if len(params.mono_in_dataset) == 0:
        return

    for lang, paths in params.mono_in_dataset.items():
        ...
        assert lang not in data['mono_in']
        data['mono_in'][lang] = {k: v for k, v in datasets}

    logger.info('')


def load_mono_out_data(params, data):
    """
    Load out-of-domain monolingual data.
    """
    assert not (len(params.mono_out_dataset) == 0) ^ (params.n_mono == 0)
    if len(params.mono_out_dataset) == 0:
        return

    for lang, paths in params.mono_out_dataset.items():
        ...
        assert lang not in data['mono_out']
        data['mono_out'][lang] = {k: v for k, v in datasets}
...
def check_all_data_params(params):
    ...
    # check in-domain monolingual datasets
    params.mono_in_dataset = {k: v for k, v in [x.split(':') for x in params.mono_in_dataset.split(';') if len(x) > 0]}
    assert not (len(params.mono_in_dataset) == 0) ^ (params.n_mono == 0)
    if len(params.mono_in_dataset) > 0:
        assert type(params.mono_in_dataset) is dict
        assert all(lang in params.langs for lang in params.mono_in_dataset.keys())
        assert all(len(v.split(',')) == 3 for v in params.mono_in_dataset.values())
        params.mono_in_dataset = {k: tuple(v.split(',')) for k, v in params.mono_in_dataset.items()}
        assert all(all(((i > 0 and path == '') or os.path.isfile(path)) for i, path in enumerate(paths))
                   for paths in params.mono_in_dataset.values())

    # check out-of-domain monolingual datasets
    params.mono_out_dataset = {k: v for k, v in [x.split(':') for x in params.mono_out_dataset.split(';') if len(x) > 0]}
    assert not (len(params.mono_out_dataset) == 0) ^ (params.n_mono == 0)
    if len(params.mono_out_dataset) > 0:
        assert type(params.mono_out_dataset) is dict
        assert all(lang in params.langs for lang in params.mono_out_dataset.keys())
        assert all(len(v.split(',')) == 3 for v in params.mono_out_dataset.values())
        params.mono_out_dataset = {k: tuple(v.split(',')) for k, v in params.mono_out_dataset.items()}
        assert all(all(((i > 0 and path == '') or os.path.isfile(path)) for i, path in enumerate(paths))
                   for paths in params.mono_out_dataset.values())

    # check in-domain parallel datasets
    params.para_in_dataset = {k: v for k, v in [x.split(':') for x in params.para_in_dataset.split(';') if len(x) > 0]}
    assert type(params.para_in_dataset) is dict
    assert all(len(k.split('-')) == 2 for k in params.para_in_dataset.keys())
    assert all(len(v.split(',')) == 3 for v in params.para_in_dataset.values())
    params.para_in_dataset = {tuple(k.split('-')): tuple(v.split(',')) for k, v in params.para_in_dataset.items()}
    for (lang1, lang2), (train_path, valid_path, test_path) in params.para_in_dataset.items():
    ...
    # check out-of-domain parallel datasets
    params.para_out_dataset = {k: v for k, v in [x.split(':') for x in params.para_out_dataset.split(';') if len(x) > 0]}
    assert type(params.para_out_dataset) is dict
    assert all(len(k.split('-')) == 2 for k in params.para_out_dataset.keys())
    assert all(len(v.split(',')) == 3 for v in params.para_out_dataset.values())
    params.para_out_dataset = {tuple(k.split('-')): tuple(v.split(',')) for k, v in params.para_out_dataset.items()}
    for (lang1, lang2), (train_path, valid_path, test_path) in params.para_out_dataset.items():
        assert lang1 < lang2 and lang1 in params.langs and lang2 in params.langs
        assert train_path == '' or os.path.isfile(train_path.replace('XX', lang1))
        assert train_path == '' or os.path.isfile(train_path.replace('XX', lang2))
        assert os.path.isfile(valid_path.replace('XX', lang1))
    ...
    assert not (params.lambda_xe_mono_in == "0" and params.lambda_xe_mono_out == "0") ^ (len(params.mono_directions) == 0)
    assert not (params.lambda_xe_para_in == "0" and params.lambda_xe_para_out == "0") ^ (len(params.para_directions) == 0)
...
def load_data(params, mono_only=False):
    ...
    """
    data = {'dico': {}, 'mono_in': {}, 'mono_out': {}, 'para_in': {}, 'para_out': {}, 'back': {}}

    if not mono_only:

        # parallel datasets
        load_para_in_data(params, data)
        load_para_out_data(params, data)

        # back-parallel datasets
        load_back_data(params, data)

    # monolingual datasets
    load_mono_in_data(params, data)
    load_mono_out_data(params, data)
    ...
    for (lang1, lang2), v in data['para_in'].items():
        for data_type in ['train', 'valid', 'test']:
            if v[data_type] is None:
                continue
            logger.info('{: <18} - {: >5} - {: >4} -> {: >4}:{: >10}'.format('In-domain parallel data', data_type, lang1, lang2, len(v[data_type])))

    for (lang1, lang2), v in data['para_out'].items():
        for data_type in ['train', 'valid', 'test']:
            if v[data_type] is None:
                continue
            logger.info('{: <18} - {: >5} - {: >4} -> {: >4}:{: >10}'.format('Out-of-domain parallel data', data_type, lang1, lang2, len(v[data_type])))


    for (lang1, lang2), v in data['back'].items():
        logger.info('{: <18} - {: >5} - {: >4} -> {: >4}:{: >10}'.format('Back-parallel data', 'train', lang1, lang2, len(v)))

    for lang, v in data['mono_in'].items():
        for data_type in ['train', 'valid', 'test']:
            logger.info('{: <18} - {: >5} - {: >12}:{: >10}'.format('In-domain monolingual data', data_type, lang, len(v[data_type]) if v[data_type] is not None else 0))

    for lang, v in data['mono_out'].items():
        for data_type in ['train', 'valid', 'test']:
            logger.info('{: <18} - {: >5} - {: >12}:{: >10}'.format('Out-of-domain monolingual data', data_type, lang, len(v[data_type]) if v[data_type] is not None else 0))
