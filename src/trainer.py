...
class TrainerMT(MultiprocessingEventLoop):
    ...
    def __init__(self, encoder, decoder, discriminator, lm, data, params):
        ...
        if params.stopping_criterion == '':
            for lang1, lang2 in self.data['para_in'].keys():
        ...
        parse_lambda_config(params, 'lambda_xe_mono_in')
        parse_lambda_config(params, 'lambda_xe_para_in')
        parse_lambda_config(params, 'lambda_xe_mono_out')
        parse_lambda_config(params, 'lambda_xe_para_out')
    ...
    def get_iterator(self, iter_name, lang1, lang2, back=False, in_domain=False):
        """
        Create a new iterator for a dataset.
        """
        assert back is False or lang2 is not None
        key = ','.join([x for x in [iter_name, lang1, lang2] if x is not None]) + ('_back' if back else '') + ('_in' if in_domain else '')
        logger.info("Creating new training %s iterator ..." % key)
        if lang2 is None:
            dataset = self.data['mono_in'][lang1]['train'] if in_domain else self.data['mono_out'][lang1]['train']
        elif back:
            dataset = self.data['back'][(lang1, lang2)]
        else:
            k = (lang1, lang2) if lang1 < lang2 else (lang2, lang1)
            dataset = self.data['para_in'][k]['train'] if in_domain else self.data['para_out'][k]['train']
        iterator = dataset.get_iterator(shuffle=True, group_by_size=self.params.group_by_size)()
        self.iterators[key] = iterator
        return iterator

    def get_batch(self, iter_name, lang1, lang2, in_domain=False, back=False):
        """
        ...
        if iterator is None:
            iterator = self.get_iterator(iter_name, lang1, lang2, back, in_domain)
        try:
            batch = next(iterator)
    ...
    def enc_dec_step(self, lang1, lang2, lambda_xe, in_domain=False, back=False):
        """
        ...
        if back:
            ...
            (sent1, len1), (sent2, len2) = self.get_batch('encdec', lang1, lang2, in_domain=in_domain)
            translation = True
        ...
        # encoded states
        encoded = self.encoder(sent1, len1, lang1_id, in_domain=in_domain, translation=translation)
